...
from network import Policy, Value, EndgamePolicy, EndgameValue
...
class Node:
    ...
    def expand(self, actions):
        if len(actions):
            policy_sum = sum(actions.values())
            for action, p in actions.iteritems():
                self.children[action] = Node(p / policy_sum)


# this should be responsible for searches in the middlegame
def search(config: Config, game: Board, network):
    root = Node(0.0)
    root.expand(actions(game, False, config))    
    while True:
        node = root
        clone = game.copy()
        search_path = [node]

        for __ in range(config.sims_between_yield):
            for _ in range(config.min_depth):
                node.expand(actions(game, False, config))
                node.to_play = game.turn
                action, node = select_child(config, node)
                search_path.append[node]
                clone.push(action)

            node.expand(actions(game, True, config))
            node.to_play = game.turn
            while node.expanded:
                action, node = select_child(config, node)
                search_path.append[node]
        ...
        val = EndgameValue(game.image()) if game.is_endgame() else Value(game.image())
        backpropogate(search_path, val, game.turn)

        yield select_action(config, game, root)

    

def select_child(config: Config, node: Node):
    children = ((UCB_score(config, node, child), action, child) for (action, child) in node.children.iteritems())
    _, action, child = max(children, key=itemgetter(0))
    return action, child
...
def actions(game: Board, prune: bool, config: Config) -> dict:
    ...
    temp = {}
    last_prior = actions[0][1] # should only be within [-1, 1]
    for number, (a, prior) in enumerate(actions):
        if (prior * config.difference_offset) <= last_prior: break
        if number > config.pruned_moves:
            temp = []
            break
        temp[a] = prior
...
def backpropogate(search_path: list[Node], value: float, to_play: bool):
    for node in search_path: 
        node.value_sum += value if node.to_play == to_play else 1-value
        node.visit_count += 1

def select_action(config: Config, game: Board, root: Node): 
